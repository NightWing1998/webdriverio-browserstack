...
if((typeof process.env.TEST_TYPE === 'undefined' && process.env.TEST_TYPE === '') || !valid_test_types.includes(process.env.TEST_TYPE)) {
  console.error("Please provide a valid TEST_TYPE when running the test.\nFor e.g. 'TEST_TYPE=single ./node_modules/.bin/wdio conf/start_tests.conf.js'\nList of all valid test types defined in conf/start_tests.conf.js are " + valid_test_types)
  process.exit(1)
}
...
for (let obj_arr of Object.entries(filtered_test_type[0]["session_caps"])) {
  ...
  for (let [key, value] of Object.entries(obj_arr[1])) {
    ...
    for (let [key, value] of Object.entries(filtered_test_type[0]["common_caps"])) {
  ...
  onPrepare: function (config, capabilities) {
    if(test_type === 'local') {
      ...
      return new Promise(function (resolve, reject) {
        ...
        for (let [key, value] of Object.entries(filtered_test_type[0]["local_binding_caps"])) {
